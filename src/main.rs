...
impl Machine {
    ...
    fn load_rom(&mut self, rom: NesRom) {
...
fn read_nes_file(path: &str) -> NesRom {
...
fn main() {
    ...
    let rom = read_nes_file("nestest.nes");
    m.load_rom(rom);
    println!("{:02X}{:02X}", m.read_mem(0xfffd), m.read_mem(0xfffc));
    let baseline = File::open("nestest.log").expect("Unable to open baseline file");
    ...
    loop {
        ...
        if baseline_line != m.get_state_string() {
            println!("Mismatch at line {}! Baseline:", line);
